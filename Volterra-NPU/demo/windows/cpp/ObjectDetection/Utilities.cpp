...
std::vector<float> LoadMat2Float(cv::Mat& image, uint32_t input_image_height, uint32_t input_image_width)
{
    if (image.empty())
    {
        std::ostringstream msg;
        msg << "Input image not loaded";
        throw std::runtime_error(msg.str());
    }

    cvtColor(image, image, cv::COLOR_BGR2RGB);
    resize(image, image, cv::Size(input_image_height, input_image_width));
    cv::Mat local_image = image.reshape(1, 1);

    std::vector<float> vec;
    local_image.convertTo(vec, CV_32FC1, 1. / 255);
...
std::vector<float> LoadImageFile(const std::string& image_path, uint32_t input_image_height, uint32_t input_image_width)
{
    ...
    std::vector<float> output = LoadMat2Float(image, input_image_height, input_image_width);
