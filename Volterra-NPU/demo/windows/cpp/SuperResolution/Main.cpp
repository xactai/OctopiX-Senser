...
void
count_time(std::chrono::steady_clock::time_point start, std::chrono::steady_clock::time_point end, std::string message)
{
    double elapsed_time = std::chrono::duration<double, std::milli>(end - start).count();
    std::cout << "******************************************************\n"
              << message << ": " << elapsed_time << "msecs" << "\n******************************************************\n";
}
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        const std::string image_pth =
            "C:\\volterra_research\\ai_apps\\ai-hub-apps\\apps\\windows\\cpp\\SuperResolution\\results\\temp.jpg";
        cv::Mat image = cv::imread(image_path, cv::IMREAD_COLOR);
        cv::resize(image, image, cv::Size(128, 128));
        cv::imwrite(image_pth, image);
        ...
        app.ProcessOutput(image_pth, output_image_path, true, "C:\\volterra_research\\ai_apps\\ai-hub-apps\\apps\\windows\\cpp\\SuperResolution\\results\\cropped_sample_5.jpg");
        end = TIME;
