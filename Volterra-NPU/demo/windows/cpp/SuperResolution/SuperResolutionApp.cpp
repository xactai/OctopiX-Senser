...
void SuperResolutionApp::LoadInputs(const std::string& image_path)
{
    ...
    std::vector<float> image_data = Utils::LoadImageFile(image_path, m_model_input_ht, m_model_input_wt, true);
...
void SuperResolutionApp::ProcessOutput(const std::string& input_image_path,
                                       ...
                                       bool display_output_image, const std::string output_path)
{
    if (m_outputs.size() != 1)
    {
        std::ostringstream err_msg;
    ...
    if (display_output_image)
    {
        ...
        /* cv::resize(output_image, output_image, cv::Size(128, 128));

        std::cout << "output cols: " << output_image.cols << "\n"
                  << "output rows: " << output_image.rows << "\n"
                  << "input cols: " << input_image.cols << "\n";*/

        std::string window_name = "Upscaled_image";

        cv::Mat output_black_box = cv::Mat::zeros(512, 512, output_image.type());
        x_offset = (output_black_box.cols - output_image.cols) / 2;
        y_offset = (output_black_box.rows - output_image.rows) / 2;
        output_image.copyTo(output_black_box(cv::Rect(x_offset, y_offset, output_image.cols, output_image.rows)));
        output_black_box.copyTo(canvas(cv::Rect(512, 0, 512, 512)));
        ...
        cv::resizeWindow(window_name, 1024, 512); // Set the window size explicitly
