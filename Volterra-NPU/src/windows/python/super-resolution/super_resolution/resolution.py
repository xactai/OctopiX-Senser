...
class SuperResolution:
    ...
    def detect_objects(self, image):
        ...
        str1 = f"prepare_input time: {(time.perf_counter() - start)*1000:.2f} ms"

        # Perform inference on the image
        start = time.perf_counter()
        outputs = self.inference(input_tensor)
        str2 = f"{BgColor.GREEN}Inference time: {(time.perf_counter() - start)*1000:.2f} ms{BgColor.OFF}"
        if isinstance(outputs, list):
            outputs = outputs[0]

        start = time.perf_counter()
        outputs = self.post_process(outputs)
        str3 = f"Post process time: {(time.perf_counter() - start)*1000:.2f} ms"
        desc = "\r" + str1 + " " + str2 + " " + str3
        print(desc, end='')
        return outputs
