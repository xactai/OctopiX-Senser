...
def main():
    ...
    text = whisper.transcribe(os.path.join(ROOT_PATH, 'assets', 'input', os.path.basename(args.audio_path)))
    print("After transcription: " + str(datetime.now().astimezone()))
    with open(os.path.join(ROOT_PATH, 'assets', 'output', os.path.basename(args.audio_path).split(".")[0]+"_out.txt"), 'w') as out_file:
        out_file.write(text)
