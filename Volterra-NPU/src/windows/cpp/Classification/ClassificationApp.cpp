...
void ClassificationApp::LoadInputImage(cv::Mat& image)
{
    ...
    std::vector<float> image_data = Utils::LoadMat2Float(image, m_model_input_ht, m_model_input_wt);
...
cv::Mat ClassificationApp::ProcessOutputImage(cv::Mat& image)
{
    if (m_outputs.size() != 1)
    {
        std::ostringstream err_msg;
    ...
    //cv::Mat input_image = cv::imread(image);
    Utils::AddText_Camera(image, class_label);
