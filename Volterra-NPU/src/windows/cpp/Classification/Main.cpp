...
void count_time(std::chrono::steady_clock::time_point start,
                std::chrono::steady_clock::time_point end,
                std::string message)
{
    const std::string div = "\n************************************************\n";
    double elapsed_time = std::chrono::duration<double, std::milli>(end - start).count();
    std::cout << div << message << elapsed_time << ": msecs" << div;
}
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        auto end = TIME;
        count_time(start, end, "Model loading time: ");

        uint16_t frame_count = 1;
        //std::string image_path = "output_image/";
        std::string output_window = "Image Classification";
        cv::Mat image;
        cv::Mat output_image;
             ...
             if (cap.isOpened())
            {
                ...
                while (true)
                {
                    ...
                    if (frame_count > 1)
                    {
                        ...
                        count_time(start, end, "input loading time: ");
                        ...
                        output_image = app.ProcessOutputImage(image);
                    ...
                    cv::imshow(output_window, output_image);
        ...
        else
        {
            ...
            start = TIME;
            app.ProcessOutput(image_path, ".\\bin\\synset.txt ");
            end = TIME;
            count_time(start, end, "End-to-End Prediction Time: ");
